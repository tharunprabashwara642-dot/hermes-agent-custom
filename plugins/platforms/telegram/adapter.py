...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not self._is_user_authorized_from_message(msg):
            self._log_blocked_user(msg)
            return
        # Gemini credential management (/gemini-key, /gemini-keys) is a deterministic,
        # code-only command — it must never reach the LLM/session pipeline below, and it
        # carries its own (stricter) authorization check independent of general bot access.
        from plugins.platforms.telegram.gemini_key_admin import maybe_handle_gemini_key_command
        if await maybe_handle_gemini_key_command(self, msg):
            return
